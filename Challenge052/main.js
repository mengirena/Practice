...
function convert(s) {
    let numbers = [0,2,3,4,5,6,7,8,9], maps = {}
    s = s.split('').map(l => l.toLowerCase()) 
    return Number(s.map((l,index) => {
        if (index === 0) { 
            maps[l] = 1
            return 1
        } else if (!(l in maps)){
            maps[l] = numbers.shift()
        }
        return maps[l]
    }).join(""))
}
...
const convert = (s) => {
    ...
    return +s.toLowerCase().replace(/./g , c=> {
      if (!map.hasOwnProperty(c)) {
        map[c] = count > 1 ? count : 1 - count; // 0 and 1 switch
        count++;
      }
      return map[c]
    })
